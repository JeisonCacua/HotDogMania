...
export default function MainMenu() {
  ...
  useEffect(() => {
    fetch("http://192.168.1.6:3001/me", { credentials: "include" })
      .then((res) => {
  ...
  useEffect(() => {
    ...
    fetch("http://192.168.1.6:3001/favoritos", { credentials: "include" })
      .then((res) => res.json())
      .then((data) => {
  ...
  const cerrarSesion = async () => {
    try {
      const response = await fetch("http://192.168.1.6:3001/logout", {
        method: "POST",
        credentials: "include",
      });
      if (response.ok) {
